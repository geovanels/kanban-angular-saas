...
var _SmtpService = class _SmtpService {
  http = inject(HttpClient);
  subdomainService = inject(SubdomainService);
  functions = inject(Functions);
  companyService = inject(CompanyService);
  authService = inject(AuthService);
  resolveCompanyWithSmtp() {
    return __async(this, null, function* () {
      let company = this.subdomainService.getCurrentCompany();
      if (company?.smtpConfig && this.validateSmtpConfig(company))
        return company;
      try {
        const sub = this.companyService.getCompanySubdomain();
        if (sub) {
          const bySub = yield this.companyService.getCompanyBySubdomain(sub);
          if (bySub?.id) {
  ...
  sendEmail(emailData) {
    return from(this.resolveCompanyWithSmtp()).pipe(switchMap((company) => {
      if (!company) {
      ...
      const payload = {
        ...
        // Cloud Function pode resolver via companyId
      ...
      return this.http.post(httpFunctionUrl, payload);
    }), map((result) => {
      return {
...
__name(_SmtpService, "SmtpService");
__publicField(_SmtpService, "\u0275fac", /* @__PURE__ */ __name(function SmtpService_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _SmtpService)();
}, "SmtpService_Factory"));
__publicField(_SmtpService, "\u0275prov", /* @__PURE__ */ \u0275\u0275defineInjectable({ token: _SmtpService, factory: _SmtpService.\u0275fac, providedIn: "root" }));
var SmtpService = _SmtpService;
(() => {
...
//# sourceMappingURL=chunk-7QBO7AZP.js.map
