...
export class KanbanComponent implements OnInit, OnDestroy {
  ...
  getCardFieldsForLead(lead: Lead): Array<{ label: string; value: any; type?: string }> {
    ...
    for (const f of fieldsToShow) {
      let value = this.readFieldValue(lead, f.apiFieldName || f.name, f.label || f.name);

      // Se for campo responsavel, converter UID para nome
      if (f.type === 'responsavel' && value) {
        const user = this.users.find(u => u.uid === value);
        value = user ? (user.displayName || user.email) : value;
      }
  ...
  getTemperatureGlobalItem(lead: Lead): { label: string; value: any } | null {
    try {
      ...
      console.error('getTemperatureGlobalItem error:', error);
      return null;
    }
  }
